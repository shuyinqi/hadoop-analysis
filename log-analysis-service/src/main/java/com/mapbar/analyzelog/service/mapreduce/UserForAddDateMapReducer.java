 ...
 * @Copyright:  图吧
 */
public class UserForAddDateMapReducer extends AbstractMapReduceJob{

	public static class UserForAddDateMapper extends TableMapper<Text, Text>{
	...
	public static class UserForAddDateReducer extends DBCounterReducer<Text, Text> {
	...
	public void run(Job job) throws IOException, InterruptedException, ClassNotFoundException {
		...
		TableMapbarReduceProxy.initTableMapperJobProxy(getHTableName(LogType.LAUNCH), scan, UserForAddDateMapper.class, 
				Text.class, Text.class, job,getArguments().getAppID(),null,2);
		job.setNumReduceTasks(3);
		JDBCMapReduceUtil.initTableReducerJob("la_channel_version_stat_add", new String[] { "date", "type", "name","version" },
				new String[] { "launch_user_count", "new_user_count" }, UserForAddDateReducer.class, job);
